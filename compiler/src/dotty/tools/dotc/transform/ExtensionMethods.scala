...
class ExtensionMethods extends MiniPhase with DenotTransformer with FullParameterization { thisPhase =>
  ...
  override def transform(ref: SingleDenotation)(implicit ctx: Context): SingleDenotation = ref match {
    ...
    case ref: SymDenotation =>
      ...
      // that will be moved to the companion object.
      if (ref.is(LocalAccess) && isDerivedValueClass(ref.owner)) {
        if (ref1 ne ref) ref1.resetFlag(LocalAccess)
        else ref1 = ref1.copySymDenotation(initFlags = ref1.flags &~ LocalAccess)
      }
      ref1
    case _ =>
      ref
